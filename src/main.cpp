...
uint256 hashGenesisBlock("0x03b52a45dcbc39a9b57f0f43312912fa3dd08cddd4ea0a46f0e709b18d786017");
...
{
    // Solarcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
if(nHeight == 1)   
		{ 
			nSubsidy = 3750000000 * COIN; 
		}
    if (nHeight > 1)
	{
        for (int Loop = 0; Loop < nHeight /10000; Loop++)
		{
...
static const int64 nTargetTimespan = 1 * 60 * 60; // Solarcoin: 1 hours
static const int64 nTargetSpacing = 2 * 60; // Solarcoin: 2 minutes
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x03b52a45dcbc39a9b57f0f43312912fa3dd08cddd4ea0a46f0e709b18d786017");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "21.06.2017 Day of the summer solstice!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041184310fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1495464276;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 9946268;
        ...
        assert(block.hashMerkleRoot == uint256("0xf4443d5569d15ada467711baed6612b9a60cfe9107b9b506ce8036210589913a"));
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc1, 0xb7, 0xdc }; // Solarcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SolarcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SolarcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SolarcoinMiner(CWallet *pwallet)
{
    printf("SolarcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("solarcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SolarcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SolarcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SolarcoinMiner, pwallet));
}
